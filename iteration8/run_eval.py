"""
Iteration 7 评估运行脚本：持续评估闭环

执行流程：查询 -> 检索 -> 生成 -> 评分 -> Faithfulness 评估

新增功能 (Iteration 7):
- 支持指定输出目录（--output-dir），生成带时间戳的结果文件
- 输出包含元数据：评估时间、corpus统计、模型配置等
- 支持 GitHub Actions 自动化触发

历史功能:
- Iteration 6: 拒答机制（基于Judge评分和Rerank分数的多层拒答）
...
使用方法:
    ...
    # 指定输出目录（用于 CI/CD）
    python run_eval.py --chunking-strategy fixed_200_40 --retrieval-mode rerank --output-dir ../data
    
    # 使用 DeepSeek Judge + 预设模式
    python run_eval.py --chunking-strategy fixed_200_40 --retrieval-mode rerank --judge-mode deepseek --rejection-preset moderate
    
    # 关闭拒答机制
    python run_eval.py --chunking-strategy fixed_200_40 --retrieval-mode rerank --no-rejection
...
from self_healing import process_self_healing, list_pending_reviews
...
def run_single_strategy(args, chunking_strategy, retrieval_mode=None):
    ...
    # ============================================================
    # Iteration 8: 自愈机制 - 检测知识库缺陷并生成审核文件
    # ============================================================
    self_healing_stats = None
    if rejection_config and 'self_healing' in rejection_config:
        self_healing_config = rejection_config['self_healing']
        self_healing_stats = process_self_healing(results, queries, self_healing_config)

    return scores, mrr_scores, precision_scores, results, rerank_analysis, faithfulness_analysis, relevance_analysis, self_healing_stats
...
def main():
    ...
    # 命令行参数解析
    ap = argparse.ArgumentParser(description="RAG 系统评估脚本（Iteration 7：持续评估闭环）")
    ...
    # 运行单个策略和检索模式
    scores, mrr_scores, precision_scores, results, rerank_analysis, faithfulness_analysis, relevance_analysis, self_healing_stats = run_single_strategy(args, args.chunking_strategy, args.retrieval_mode)
    ...
    # 文件名包含 chunking 策略、检索模式和 rerank
    rerank_suffix = f"_rerank_{args.rerank_mode}" if args.rerank_mode != "none" else ""
    output_file = os.path.join(
        args.output_dir,
        f"results_{args.chunking_strategy}_{args.retrieval_mode}{rerank_suffix}_{timestamp}.json"
    )
    
    # 统计 corpus 信息
    import glob
    corpus_files = glob.glob(os.path.join(args.corpus_dir, "doc-*.txt"))
    ...
    result_data = {
        "metadata": {
            ...
            "corpus_stats": {
                "num_documents": len(corpus_files),
    ...
    # Iteration 8: 保存自愈统计
    if self_healing_stats:
        result_data["self_healing"] = self_healing_stats
    ...
    print(f"   - Documents: {len(corpus_files)}")
    print(f"   - Queries: {len(queries)}")
    print(f"   - Config: {args.chunking_strategy} + {args.retrieval_mode} + {args.rerank_mode}")
