...
**Citations 字段说明**：
- span: 必须是 answer 中的**完整连续子串**（逐字匹配，不能改写）
- source: 必须是资料中真实存在的标签（如 [文档13:片段1]），**不要编造标签**
- 每个 span 应该尽量保留原文关键词，便于后续验证
- 如果某句话综合了多个片段，可以为同一个 span 标注多个 source

**重要约束**：
- span 必须从 answer 中**逐字复制粘贴**，不要改写或总结
- 例如：如果 answer 写的是"在 APP 中开启"，span 就必须是"在 APP 中开启"，不能改成"开启后"
- 先完整生成 answer，再从 answer 中提取 span
